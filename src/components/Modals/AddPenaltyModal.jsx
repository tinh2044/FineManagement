...
import { addPenaltyToLegalCase } from "../../services";
import CloseIcon from "@mui/icons-material/Close";
import { toastMessage } from "../../context/ToastProvider";
import { useSelector } from "react-redux";
const AddPenaltyModal = ({ open, handleClose, prisoner, caseId, onAdd }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            console.log(caseId, penaltyData);
            await addPenaltyToLegalCase(caseId, {
                penaltyTypeId: penaltyData.penaltyTypeId,
                penaltyAmount: parseInt(penaltyData.penaltyAmount),
            });
            toastMessage(" Thêm án phạt thành công!", "success");
            handleClose();
        } catch (error) {
            console.log(error);
            toastMessage("Lỗi khi thêm án phạt.", "error");
            setLoading(false);
        }
        setPenaltyData({
            penaltyTypeId: "",
    ...
    return (
        <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
            <DialogTitle className="flex justify-between items-center">
                <span className="text-xl font-semibold">Thêm án phạt cho {prisoner.prisonerName}</span>
